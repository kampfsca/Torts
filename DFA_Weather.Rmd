...
$$x_t = x_{t-1}+w_t \text{ where } w_t \sim \text{MVN}(0,\bf{Q}) $$ $$y_t = Zx_{t-1}+a+v_t \text{ where } w_t \sim \text{MVN}(0,\bf{Q}) $$ $$x_0 \sim \text{MVN}(\pi,\Lambda)$$
...
Examination of the meterological variables showed very close similarities across weather stations, and some value discrepancies in Daylight. Therefore, the average for each meteorological variable was taken from the four weather stations to create single data vector for each covariate, thus eliminating any additional noise from individual weather stations. This was preferable to subsetting the patient data to their respective weather station and running separate models.

The discrepancies in the Daylight were troublesome, and believed to be erroneous. Therefore, hours of sunlight was taken from an outside data source for the specific centroid of Grand Rapids, MI. ![](Wx.png)
